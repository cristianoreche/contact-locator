<x-layout.auth>
    ...
    <div class="uc-auth">
        ...
        <x-form.form method="POST" action="{{ route('verification.send') }}">
            <x-form.button full="true">Reenviar E-mail de Verificação</x-form.button>
        </x-form.form>

        <p class="uc-auth__text mt-4">
            <x-form.link href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
                Sair da Conta
            </x-form.link>
        </p>
